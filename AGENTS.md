...
- Testimonials are two compact, equal-width cards using the active theme (`--color-surface`, `--color-text`, `--color-muted`), stacked below 700px. Keep supplied brand logos intact in small square frames, the business name above “Client testimonial”, a short quote, and five stars without a numeric score. Softpoint's existing wording/attribution is approved by Felix. Sayu's requested five-star example is labeled sample wording/rating, not client-submitted; never silently convert it into a verified endorsement. Source content from `src/content/testimonials.ts`; its CSS module owns all card styling.
- Lead selected work with Softpoint Enterprise (Feb 2026, Full Stack Developer), its supplied screenshots, Next.js/Supabase/Vercel/Gemini API stack, live website, and the supplied 30% administrative-workload reduction. Keep the outcome attributed to Felix's supplied project facts in source documentation; do not invent further metrics or AI behavior. Follow it with the existing Sayu project study using its supplied logo explicitly labeled as a brand preview, not an application screenshot. Remaining concepts stay labeled and collapsed.
- Alternate featured rows by their index in `.featured-list`: first preview left/copy right, next preview right/copy left. Stack preview above copy below 900px. Project actions are quiet underlined text links with small directional arrows, not competing filled pills. Keep portfolio work styles in their CSS module and facts in typed content.
- `ProjectPreviewGallery` owns fixed-ratio, uncropped previews, pressed-state manual controls, and a restrained 3D deck. Rotate loaded screenshots every 4.4 seconds only while visible and idle; pause on hover, focus, hidden tabs, expansion, explicit pause, and reduced motion. Manual selection remains paused until resumed. Use a 750–900 ms opacity/transform transition, at most four degrees of mouse tilt with requestAnimationFrame, no touch tilt, and no animation dependencies. Single-image galleries do not autoplay.
- Click/tap or keyboard activation opens the shared native dialog with full-size contained images, close control, Escape dismissal, focus containment, and return focus. Never mutate body/html overflow for the gallery; verify scrolling after repeated open/close cycles. Do not couple gallery state to the startup loader or skills ribbon.
- Contact retains the mailto CTA and readable address with an explicit copy button. Announce success only after clipboard completion and expose manual-copy guidance on failure. Footer email/phone are monochrome, theme-aware, labeled icon links with 44px targets; the back-to-top arrow animates once on hover/focus and returns to `/#hero` from case studies as well. Respect reduced motion.
