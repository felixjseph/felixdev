...
- Keep the skills ribbon paused and visually hidden while the startup loader is present, including its exit fade. When the loader is removed, reveal the ribbon with a short opacity/translation transition and resume the loop immediately. Use the loader's actual DOM lifecycle rather than a separate delay or document scroll lock. Reduced-motion users get an immediate reveal and the existing static, horizontally scrollable ribbon.
