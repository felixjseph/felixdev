...
- About: keep “Where work slows down” inside a centered, compact editorial frame. Tighten its ledger and method spacing at every ratio; standard phones retain horizontal index/source/result rows and use a two-by-two Understand → Build → Connect → Improve sequence, with a single-column fallback only below 301px.
- Skills: place the focused “Daily drivers. Built for the work.” chapter at the dedicated `#skills` anchor, after the Friction to Flow About section. Use one seamless icon-only monochrome rail for Claude Code, Codex, Cursor, Visual Studio Code, Zapier, Make.com, Activepieces, Vercel, Docker, and Google Gemini rather than grouped rows, labels, colored technology pills, or the superseded Signal Map. Each official brand shape links to its verified website in a new tab and retains an accessible label and keyboard focus treatment. Give every mark the same circular hover target and restrained inversion, keep the spacing identical through each loop seam, pause on hover/focus, and become static and horizontally scrollable under reduced motion.
- Selected work: borderless, background-free alternating rows float on the page with a fluid 5.5–9rem separation. Softpoint screenshots lead on the left; the existing Sayu study reverses the arrangement; Solara closes the featured sequence on the left with three supplied production screenshots, Web Developer credit, and live-site/case-study actions. Every homepage description uses one concise, professional paragraph sourced only from approved facts; Softpoint’s approved 30% result belongs naturally inside that story, while the display metric remains in the case study. Do not show temporary asset-status notices beneath Sayu. The earlier concept-exploration rows are removed. Below 900px, previews stack above a left-aligned, max-34rem copy block; its eyebrow, title, role, technologies, and actions share the same reading edge while the deck remains centered. Minimal underlined actions keep visual priority on the work.
- Gallery: show three physical media layers inside a deliberately smaller preview footprint with an editorial gap before its description. Homepage galleries omit both the caption/count row and manual selector strip, placing only a compact pause/resume control over the image; full case studies retain their captions and selectors. The resting deck uses a restrained 3D tilt; hover/focus faces the main card forward, fans the rear pair to opposite sides with about 20–30% exposure, and adds capped cursor response plus a soft radial shine. At 900px and below, crossing the central viewport band activates a slightly tighter fan automatically so touch layouts never depend on hover. Real media rotates forward every 4.4 seconds; single-image projects repeat that approved visual only as decorative hidden-from-assistive-technology rear layers. Pause on all interaction, hidden/offscreen states, and reduced motion. The accessible expanded viewer uses overlaid previous/next arrows plus keyboard Left/Right navigation, with no caption breadcrumbs, and never mutates page overflow.
- Case studies: Softpoint Enterprise and Solara share one compact editorial system—quiet project metadata, lead statement, wide uncropped gallery, stack row, and numbered chapters—while retaining their own approved facts and preview backgrounds.
- Services: introduce the chapter with “What I Can Do,” then retain the sticky-left/open-timeline composition and its scroll- and pointer-responsive hairline, with five concise delivery stages—Discovery, Plan, Build, Test, and Launch & Support. The chapter explains how Felix helps through workflow automation, AI chatbots, and web development. Each circular milestone keeps its stage glyph visible while a small completion badge draws at the upper-right when reached or hovered, borrowing the startup loader’s completion language without replaying it literally. Finish with only those three service chips; preserve the fully drawn reduced-motion state and avoid a second card grid.
- Testimonials: a responsive 1.5–2.5rem lead-in supplements the surrounding section padding, creating a deliberate but restrained break after the final project. Show one compact same-theme entry at a time in a centered hairline carousel capped at 54rem, using 2.5rem logos, restrained stars, a small quote, counter, navigation, and pause/resume. On mobile, the heading stays left while the controls occupy the right side of the same row; the brand and quote follow the same left reading edge within a 12rem track. Auto-advance every 6.2 seconds only while visible and idle. Softpoint and Sayu are approved five-star client testimonials. Each active quote receives a short character reveal after entering view; assistive technology receives the complete sentence and reduced motion renders it immediately.
- Contact and footer: restore direct contact details with no copy-email control. Keep the send-email CTA. The contact headline uses a slower word-safe character typing entrance with a complete accessible label and static fallback, followed by a caret that appears only after the phrase is complete and continues in a restrained loop that pauses offscreen or when the document is hidden. Its invitation is edited into two concise lines for a calmer tablet and desktop composition. Footer email, phone, and back-to-top arrow circles share the same 2.75rem visual size and accessible labels; motion is removed when reduced motion is requested.
- Performance: preserve the rendered presentation while frame-capping cursor and timeline work, updating animation variables directly rather than re-rendering React trees, scoping pointer listeners to their owning section, suspending persistent motion offscreen/when hidden, and keeping cleanup explicit. Optimize source assets at their real display resolution without changing the approved artwork.
- The official CV is downloadable from the navigation in desktop and mobile layouts. Keep the supplied PDF intact.
- Navigation wording: label the project destination “Work,” retain “Resume” in the compact desktop action, and use “Download CV” in the expanded mobile menu. The hero motion control sits above the lower viewport edge with fluid 1–1.6rem clearance.
...
### About Felix Portrait Chapter

The personal chapter sits directly after Services and is deliberately absent from primary navigation. It combines a compact Nohemi/editorial introduction, three friendly illustrated belief tiles, and a supplied portrait in a balanced split layout. The complete chapter uses a responsive 2.5–4.5rem total inset and a slightly narrower content cap so every breakpoint retains deliberate edge clearance. The portrait stays modest in scale, remains centered within its column, and gains an additional 1.5rem inset when the layout stacks. Its restrained frame begins monochrome. On fine-pointer hover, a soft, low-saturation elliptical color bloom follows a minimal `+` cursor with frame-capped interpolation; the native cursor disappears only over the image. Touch users can tap or drag the same feathered reveal without losing vertical scrolling, and the preview dismisses automatically. Reduced motion removes interpolation without hiding content. The portrait uses the supplied source image, responsive `object-fit` cropping, and an accessible descriptive alt rather than a generated substitute.

An understated green availability signal may identify “Open to work,” but it must remain secondary to the personal story and must not imply unverified availability details, rates, clients, or outcomes.
