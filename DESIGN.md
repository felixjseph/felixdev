...
- Hero identity: restore the original mono name · role label and green pulse. A two-column inline grid centers the dot beside the complete text block; on phones, name and role stack naturally within that block without pushing the dot to the viewport edge. Supporting copy remains “Useful software, thoughtful design, and fewer unnecessary steps.” The pulse follows the hero's motion controls.
- Selected work: compact alternating rows, starting with Softpoint screenshots on the left and its supplied role/date, stack, 30% outcome, and case study on the right. The existing Sayu study follows with copy left and its supplied brand identity right (explicitly not a screenshot). Mobile stacks preview then copy. Unverified concepts remain small disclosure rows. Minimal underlined actions and small arrows replace filled project buttons.
- Gallery: original images remain uncropped in a fixed-ratio deck. Loaded frames rotate every 4.4 seconds with a 750–900 ms opacity/transform transition. A capped four-degree mouse tilt adds depth; touch stays flat. Autoplay pauses on interaction, focus, hidden tabs, offscreen state, manual selection, explicit pause, and reduced motion. Expansion opens an accessible native dialog with full-size images, a close control, Escape support, Tab wrapping, and return focus. Never mutate document overflow or interfere with startup/ribbon timing.
- Testimonials: two compact same-theme cards, two columns above 700px and stacked below. Supplied square business logos, name over “Client testimonial”, prominent gold stars without numerical scores, and short readable quotes. Softpoint's existing wording/attribution is approved by Felix. The requested Sayu example and five-star rating remain clearly labeled as sample, not client-submitted. Use typed local content and a single CSS module.
- Contact: keep the send-email CTA and clickable readable address; add explicit clipboard copy with accessible success/error feedback. Footer contact links use monochrome icons, accessible names, and 44px targets. Its back-to-top arrow returns smoothly on hover/focus; no animation under reduced motion.
