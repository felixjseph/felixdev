...
- Experience: preserve the animated sticky “Built to adapt” statement on the left. The right column is a concise two-entry timeline sourced from the official CV, with company, role, period, work mode, two evidence-led highlights, and a restrained skill row. Core competencies follow as a single wrapping chip group rather than a second card grid.
