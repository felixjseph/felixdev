...
The personal chapter sits directly after Services and is deliberately absent from primary navigation. It combines a large Nohemi/editorial introduction, three open belief rows, and a supplied portrait in a single balanced split layout. The portrait begins monochrome; on fine-pointer hover, a frame-capped CSS aperture follows the cursor and reveals the original color locally. Touch layouts keep the calm monochrome image, while reduced motion removes interpolation without hiding content. The portrait uses the supplied source image, responsive `object-fit` cropping, and an accessible descriptive alt rather than a generated substitute.
