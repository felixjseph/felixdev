...
The personal chapter sits directly after Services and is deliberately absent from primary navigation. It combines a compact Nohemi/editorial introduction, three friendly illustrated belief tiles, and a supplied portrait in a balanced split layout. The portrait stays modest in scale with restrained corners and begins monochrome. On fine-pointer hover, a small rounded-rectangle color viewfinder follows a minimal `+` cursor with frame-capped interpolation; the native cursor disappears only over the image. Touch users can tap or drag the same viewfinder without losing vertical scrolling, and the preview dismisses automatically. Reduced motion removes interpolation without hiding content. The portrait uses the supplied source image, responsive `object-fit` cropping, and an accessible descriptive alt rather than a generated substitute.
