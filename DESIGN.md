...
- Selected work: borderless, background-free alternating rows float on the page with a fluid 5.5–9rem separation. Softpoint screenshots lead on the left; the existing Sayu study reverses the arrangement; Solara closes the featured sequence on the left with three supplied production screenshots, Web Developer credit, and live-site/case-study actions. Every homepage description uses one concise, professional paragraph sourced only from approved facts; Softpoint’s approved 30% result belongs naturally inside that story, while the display metric remains in the case study. Do not show temporary asset-status notices beneath Sayu. The earlier concept-exploration rows are removed. Below 900px, previews stack above centered, max-34rem copy blocks; at 540px and below, body descriptions align left while the surrounding metadata and actions retain their centered hierarchy. Minimal underlined actions keep visual priority on the work.
...
- Testimonials: a responsive 1.5–2.5rem lead-in supplements the surrounding section padding, creating a deliberate but restrained break after the final project. Show one compact same-theme entry at a time in a centered hairline carousel capped at 54rem, using 2.5rem logos, restrained stars, a small quote, counter, navigation, and pause/resume. The mobile composition centers its heading, controls, brand, and quote within a 12rem track. Auto-advance every 6.2 seconds only while visible and idle. Softpoint and Sayu are approved five-star client testimonials. Each active quote receives a short character reveal after entering view; assistive technology receives the complete sentence and reduced motion renders it immediately.
