...
- Experience: preserve the animated sticky “Built to adapt” statement on the left at a calmer scale. The right column is a concise, card-free two-entry timeline sourced from the official CV, with company, role, period, work mode, short factual paragraphs, and a restrained skill row. One continuous hairline connects circular milestones; it fills with viewport progress on desktop and mobile, follows pointer height only when a mouse is available, and renders complete under reduced motion. Core competencies follow as a single wrapping chip group without a top divider or a second card grid.
...
- Navigation wording: retain “Resume” in the compact desktop action and use “Download CV” in the expanded mobile menu. The hero motion control sits above the lower viewport edge with fluid 1–1.6rem clearance.
